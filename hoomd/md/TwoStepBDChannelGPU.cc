...
#include "TwoStepBDChannelGPU.h"
#include "TwoStepBDChannelGPU.cuh"
...
/*! \file TwoStepBDChannelGPU.h
    \brief Contains code for the TwoStepBDChannelGPU class
*/
...
*/
TwoStepBDChannelGPU::TwoStepBDChannelGPU(std::shared_ptr<SystemDefinition> sysdef,
    ...
    if (!m_exec_conf->isCUDAEnabled())
        {
        m_exec_conf->msg->error() << "Creating a TwoStepBDChannelGPU while CUDA is disabled" << endl;
        throw std::runtime_error("Error initializing TwoStepBDChannelGPU");
        }
...
*/
void TwoStepBDChannelGPU::integrateStepOne(unsigned int timestep)
    {
    // profile this step
    if (m_prof)
    ...
    gpu_brownian_channel_step_one(d_pos.data,
                          d_vel.data,
                          d_image.data,
                          box,
                          d_diameter.data,
...
*/
void TwoStepBDChannelGPU::integrateStepTwo(unsigned int timestep)
    {
    // there is no step 2
    }

void export_TwoStepBDChannelGPU(py::module& m)
    {
    py::class_<TwoStepBDChannelGPU, std::shared_ptr<TwoStepBDChannelGPU> >(m, "TwoStepBDChannelGPU", py::base<TwoStepBDChannel>())
