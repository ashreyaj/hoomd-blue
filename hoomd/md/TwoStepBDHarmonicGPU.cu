...
/*! \file TwoSteBDHarmonicGPU.cu
    \brief Defines GPU kernel code for Brownian integration on the GPU. Used by TwoStepBDHarmonicGPU.
*/
...
/*! \param d_pos array of particle positions and types
    ...
    \param k Spring constant of harmonic potential
    \param y0 Potential minimum
...
*/
extern "C" __global__
void gpu_brownian_harmonic_step_one_kernel(Scalar4 *d_pos,
                                  Scalar4 *d_vel,
                                  int3 *d_image,
                                  const BoxDim box,
    ...
    if (local_idx < nwork)
	    ...
	    RandomGenerator rng(RNGIdentifier::TwoStepBDHarmonic, seed, ptag, timestep);
		...
		else
		{
		    ...
		    // update position
            postype.x += (net_force.x + Fr_x) * deltaT / gamma;
            postype.y += (net_force.y + Fr_y - d_k * (postype.y - d_y0)) * deltaT / gamma;
		    ...
		    vel.y = (net_force.y- d_k * (postype.y - d_y0))/gamma;
		    vel.z = Scalar(0.0);
		}
...
/*! \param d_pos array of particle positions and types
    ...
    \param deltaT Amount of real time to step forward in one time step
    \param D Dimensionality of the system
    \param d_k Spring constant of harmonic potential
    \param d_y0 Potential minimum
    \param d_noiseless_t If set true, there will be no translational noise (random force)
    \param d_noiseless_r If set true, there will be no rotational noise (random torque)

    This is just a driver for gpu_brownian_step_one_kernel(), see it for details.
*/
cudaError_t gpu_brownian_harmonic_step_one(Scalar4 *d_pos,
                                  Scalar4 *d_vel,
                                  int3 *d_image,
                                  const BoxDim& box,
                                  const Scalar *d_diameter,
    ...
    for (int idev = gpu_partition.getNumActiveGPUs() - 1; idev >= 0; --idev)
        ...
        // run the kernel
        gpu_brownian_harmonic_step_one_kernel<<< grid, threads, (unsigned int)(sizeof(Scalar)*langevin_args.n_types + sizeof(Scalar3)*langevin_args.n_types)>>>
                                    (d_pos,
                                     d_vel,
                                     d_image,
                                     box,
