...
class whdf(pair):
    ...
    :py:class:`whdf` specifies that a WHDF pair potential should be applied between every
    non-excluded particle pair in the simulation.
    ...
    def __init__(self, r_cut, nlist, name=None):
        ...
        # create the c++ mirror class
        if not hoomd.context.exec_conf.isCUDAEnabled():
            self.cpp_force = _md.PotentialPairWHDF(hoomd.context.current.system_definition, self.nlist.cpp_nlist, self.name);
            self.cpp_class = _md.PotentialPairWHDF;
        else:
            self.nlist.cpp_nlist.setStorageMode(_md.NeighborList.storageMode.full);
            self.cpp_force = _md.PotentialPairWHDFGPU(hoomd.context.current.system_definition, self.nlist.cpp_nlist, self.name);
            self.cpp_class = _md.PotentialPairWHDFGPU;
