...
#include "TwoStepBDConstrainToXGPU.cuh"
...
/*! \file TwoStepBDConstrainToXGPU.h
    \brief Contains code for the TwoStepBDConstrainToXGPU class
*/
...
*/
TwoStepBDConstrainToXGPU::TwoStepBDConstrainToXGPU(std::shared_ptr<SystemDefinition> sysdef,
                           ...
                           bool noiseless_t,
                           bool noiseless_r)
    : TwoStepBDConstrainToX(sysdef, group, T, seed, use_lambda, lambda, noiseless_t, noiseless_r)
    {
    if (!m_exec_conf->isCUDAEnabled())
        {
        m_exec_conf->msg->error() << "Creating a TwoStepBDConstrainToXGPU while CUDA is disabled" << endl;
        throw std::runtime_error("Error initializing TwoStepBDConstrainToXGPU");
        }
...
*/
void TwoStepBDConstrainToXGPU::integrateStepOne(unsigned int timestep)
    {
    // profile this step
    if (m_prof)
    ...
    gpu_brownian_constraintox_step_one(d_pos.data,
                          d_vel.data,
                          d_image.data,
                          box,
                          d_diameter.data,
...
*/
void TwoStepBDConstrainToXGPU::integrateStepTwo(unsigned int timestep)
    {
    // there is no step 2
    }

void export_TwoStepBDConstrainToXGPU(py::module& m)
    {
    py::class_<TwoStepBDConstrainToXGPU, std::shared_ptr<TwoStepBDConstrainToXGPU> >(m, "TwoStepBDConstrainToXGPU", py::base<TwoStepBDConstrainToX>())
