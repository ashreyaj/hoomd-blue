...
/*! \file TwoStepABDMobilityFieldGPU.cu
    \brief Defines GPU kernel code for Brownian integration on the GPU. Used by TwoStepABDMobilityFieldGPU.
*/
        ...
        if (aniso)
            ...
            if (gamma_r.x > 0 || gamma_r.y > 0 || gamma_r.z > 0)
				...
				else
                {
					...
					Scalar sigma_r = fast::sqrt(Scalar(2.0)*T/(mu_r*deltaT));
	            	if (d_noiseless_r)
	                	sigma_r = Scalar(0.0);
...
/*! \param d_pos array of particle positions and types
    ...
    This is just a driver for gpu_active_brownian_mobilityfield_step_one_kernel(), see it for details.
*/
