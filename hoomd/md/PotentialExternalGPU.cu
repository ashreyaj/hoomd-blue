...
//! Evaluator for WHDFfinite pair potential.
template cudaError_t gpu_cpef<EvaluatorWalls<EvaluatorPairWHDFfinite> >(const external_potential_args_t& external_potential_args, const typename EvaluatorWalls<EvaluatorPairWHDFfinite>::param_type *d_params, const typename EvaluatorWalls<EvaluatorPairWHDFfinite>::field_type *d_field);
