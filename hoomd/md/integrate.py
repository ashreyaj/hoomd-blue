...
class active_brownian_mobilityfield(_integration_method):
    def __init__(self, group, kT, seed, dscale=False, noiseless_t=False, noiseless_r=False, rcon=1.0, alpha=1.0, v0=1.0, td_field=True, rd_field=True):
        hoomd.util.print_status_line();
        ...
        # initialize the reflected c++ class
        if not hoomd.context.exec_conf.isCUDAEnabled():
            my_class = _md.TwoStepABDMobilityField;
        else:
            my_class = _md.TwoStepABDMobilityFieldGPU;
        ...
        self.metadata_fields = ['group', 'kT', 'seed', 'dscale','noiseless_t','noiseless_r', 'rcon', 'alpha', 'v0', 'td_field', 'rd_field']
...
class brownian_anisogamma(_integration_method):
    def __init__(self, group, kT, seed, dscale=False, noiseless_t=False, noiseless_r=False):
        hoomd.util.print_status_line();
        ...
        # initialize the reflected c++ class
        if not hoomd.context.exec_conf.isCUDAEnabled():
            my_class = _md.TwoStepBDAnisoGamma;
        else:
            my_class = _md.TwoStepBDAnisoGammaGPU;
...
class brownian_gaussian(_integration_method):
    R""" Brownian dynamics with Gaussian noise.
...
class brownian_constraintox(_integration_method):
    def __init__(self, group, kT, seed, dscale=False, noiseless_t=False, noiseless_r=False):
        hoomd.util.print_status_line();
        ...
        # initialize the reflected c++ class
        if not hoomd.context.exec_conf.isCUDAEnabled():
            my_class = _md.TwoStepBDConstrainToX;
        else:
            my_class = _md.TwoStepBDConstrainToXGPU;
...
class brownian_channel(_integration_method):
    def __init__(self, group, kT, seed, dscale=False, fconst=1.0, y0=0.0, width=1.0, noiseless_t=False, noiseless_r=False):
        ...
        # initialize the reflected c++ class
        if not hoomd.context.exec_conf.isCUDAEnabled():
            my_class = _md.TwoStepBDChannel;
        else:
            my_class = _md.TwoStepBDChannelGPU;
