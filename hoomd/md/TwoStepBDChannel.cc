...
/*! \file TwoStepBDChannel.h
    \brief Contains code for the TwoStepBDChannel class
*/
...
*/
TwoStepBDChannel::TwoStepBDChannel(std::shared_ptr<SystemDefinition> sysdef,
...
TwoStepBDChannel::~TwoStepBDChannel()
    {
    m_exec_conf->msg->notice(5) << "Destroying TwoStepBDChannel" << endl;
    }
...
*/
void TwoStepBDChannel::integrateStepOne(unsigned int timestep)
    {
    ...
    for (unsigned int group_idx = 0; group_idx < group_size; group_idx++)
        ...
        // Initialize the RNG
        RandomGenerator rng(RNGIdentifier::TwoStepBDChannel, m_seed, ptag, timestep);
...
/*! \param timestep Current time step
*/
void TwoStepBDChannel::integrateStepTwo(unsigned int timestep)
    {
    // there is no step 2 in Brownian dynamics.
    }

void export_TwoStepBDChannel(py::module& m)
    {
    py::class_<TwoStepBDChannel, std::shared_ptr<TwoStepBDChannel> >(m, "TwoStepBDChannel", py::base<TwoStepLangevinBase>())
