...
/*! \file TwoStepBDGaussianGPU.h
    \brief Contains code for the TwoStepBDGaussianGPU class
*/
...
*/
TwoStepBDGaussianGPU::TwoStepBDGaussianGPU(std::shared_ptr<SystemDefinition> sysdef,
                           ...
                           bool noiseless_t,
                           bool noiseless_r)
    : TwoStepBDGaussian(sysdef, group, T, seed, use_lambda, lambda, noiseless_t, noiseless_r)
    {
    if (!m_exec_conf->isCUDAEnabled())
        {
        m_exec_conf->msg->error() << "Creating a TwoStepBDGaussianGPU while CUDA is disabled" << endl;
        throw std::runtime_error("Error initializing TwoStepBDGaussianGPU");
        }
...
*/
void TwoStepBDGaussianGPU::integrateStepOne(unsigned int timestep)
    {
    // profile this step
    if (m_prof)
    ...
    gpu_brownian_gaussian_step_one(d_pos.data,
                          d_vel.data,
                          d_image.data,
                          box,
                          d_diameter.data,
...
*/
void TwoStepBDGaussianGPU::integrateStepTwo(unsigned int timestep)
    {
    // there is no step 2
    }

void export_TwoStepBDGaussianGPU(py::module& m)
    {
    py::class_<TwoStepBDGaussianGPU, std::shared_ptr<TwoStepBDGaussianGPU> >(m, "TwoStepBDGaussianGPU", py::base<TwoStepBDGaussian>())
