...
/*! \file TwoSteBDConstrainToXGPU.cu
    \brief Defines GPU kernel code for Brownian integration on the GPU. Used by TwoStepBDConstrainToXGPU.
*/
...
*/
extern "C" __global__
void gpu_brownian_constraintox_step_one_kernel(Scalar4 *d_pos,
                                  Scalar4 *d_vel,
                                  int3 *d_image,
                                  const BoxDim box,
    ...
    if (local_idx < nwork)
	    ...
	    RandomGenerator rng(RNGIdentifier::TwoStepBDConstrainToX, seed, ptag, timestep);
...
/*! \param d_pos array of particle positions and types
    ...
    This is just a driver for gpu_brownian_constraintox_step_one_kernel(), see it for details.
*/
cudaError_t gpu_brownian_constraintox_step_one(Scalar4 *d_pos,
                                  Scalar4 *d_vel,
                                  int3 *d_image,
                                  const BoxDim& box,
                                  const Scalar *d_diameter,
    ...
    for (int idev = gpu_partition.getNumActiveGPUs() - 1; idev >= 0; --idev)
        ...
        // run the kernel
        gpu_brownian_constraintox_step_one_kernel<<< grid, threads, (unsigned int)(sizeof(Scalar)*langevin_args.n_types + sizeof(Scalar3)*langevin_args.n_types)>>>
                                    (d_pos,
                                     d_vel,
                                     d_image,
                                     box,
