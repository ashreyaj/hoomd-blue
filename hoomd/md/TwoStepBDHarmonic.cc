...
/*! \file TwoStepBDHarmonic.h
    \brief Contains code for the TwoStepBDHarmonic class
*/
...
/*! \param sysdef SystemDefinition this method will act on. Must not be NULL.
    ...
    \param k Spring constant of potential
...
*/
TwoStepBDHarmonic::TwoStepBDHarmonic(std::shared_ptr<SystemDefinition> sysdef,
  ...
  : TwoStepLangevinBase(sysdef, group, T, seed, use_lambda, lambda),
    m_k(k), m_y0(y0), m_noiseless_t(noiseless_t), m_noiseless_r(noiseless_r)
    {
    m_exec_conf->msg->notice(5) << "Constructing TwoStepBDHarmonic" << endl;
    }

TwoStepBDHarmonic::~TwoStepBDHarmonic()
    {
    m_exec_conf->msg->notice(5) << "Destroying TwoStepBDHarmonic" << endl;
    }
...
*/
void TwoStepBDHarmonic::integrateStepOne(unsigned int timestep)
    {
    ...
    for (unsigned int group_idx = 0; group_idx < group_size; group_idx++)
        ...
        // Initialize the RNG
        RandomGenerator rng(RNGIdentifier::TwoStepBDHarmonic, m_seed, ptag, timestep);
        ...
        h_pos.data[j].y += (h_net_force.data[j].y + Fr_y - m_k*(h_net_force.data[j].y - m_y0))*m_deltaT/gamma;
        ...
        h_vel.data[j].y = (h_net_force.data[j].y- m_k*(h_net_force.data[j].y - m_y0))/gamma;
...
/*! \param timestep Current time step
*/
void TwoStepBDHarmonic::integrateStepTwo(unsigned int timestep)
    {
    // there is no step 2 in Brownian dynamics.
    }

void export_TwoStepBDHarmonic(py::module& m)
    {
    py::class_<TwoStepBDHarmonic, std::shared_ptr<TwoStepBDHarmonic> >(m, "TwoStepBDHarmonic", py::base<TwoStepLangevinBase>())
