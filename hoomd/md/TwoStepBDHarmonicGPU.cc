...
#include "TwoStepBDHarmonicGPU.h"
#include "TwoStepBDHarmonicGPU.cuh"
...
/*! \file TwoStepBDHarmonicGPU.h
    \brief Contains code for the TwoStepBDHarmonicGPU class
*/
...
*/
TwoStepBDHarmonicGPU::TwoStepBDHarmonicGPU(std::shared_ptr<SystemDefinition> sysdef,
                           ...
                           bool noiseless_t,
                           bool noiseless_r)
    : TwoStepBDHarmonic(sysdef, group, T, seed, use_lambda, lambda, k, y0, noiseless_t, noiseless_r)
    {
    if (!m_exec_conf->isCUDAEnabled())
        {
        m_exec_conf->msg->error() << "Creating a TwoStepBDHarmonicGPU while CUDA is disabled" << endl;
        throw std::runtime_error("Error initializing TwoStepBDHarmonicGPU");
        }
...
*/
void TwoStepBDHarmonicGPU::integrateStepOne(unsigned int timestep)
    {
    // profile this step
    if (m_prof)
    ...
    gpu_brownian_harmonic_step_one(d_pos.data,
                          d_vel.data,
                          d_image.data,
                          box,
                          d_diameter.data,
...
*/
void TwoStepBDHarmonicGPU::integrateStepTwo(unsigned int timestep)
    {
    // there is no step 2
    }

void export_TwoStepBDHarmonicGPU(py::module& m)
    {
    py::class_<TwoStepBDHarmonicGPU, std::shared_ptr<TwoStepBDHarmonicGPU> >(m, "TwoStepBDHarmonicGPU", py::base<TwoStepBDHarmonic>())
