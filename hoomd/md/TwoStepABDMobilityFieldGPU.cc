...
/*! \file TwoStepABDMobilityFieldGPU.h
    \brief Contains code for the TwoStepABDMobilityFieldGPU class
*/
...
*/
TwoStepABDMobilityFieldGPU::TwoStepABDMobilityFieldGPU(std::shared_ptr<SystemDefinition> sysdef,
                           ...
                           bool td_field,
                           bool rd_field)
    : TwoStepABDMobilityField(sysdef, group, T, seed, use_lambda, lambda, noiseless_t, noiseless_r, rcon, alpha, v0, td_field, rd_field)
    {
    if (!m_exec_conf->isCUDAEnabled())
        {
        m_exec_conf->msg->error() << "Creating a TwoStepABDMobilityFieldGPU while CUDA is disabled" << endl;
        throw std::runtime_error("Error initializing TwoStepABDMobilityFieldGPU");
        }
...
*/
void TwoStepABDMobilityFieldGPU::integrateStepOne(unsigned int timestep)
    {
    // profile this step
    if (m_prof)
    ...
    gpu_active_brownian_mobilityfield_step_one(d_pos.data,
                          d_vel.data,
                          d_image.data,
                          box,
                          d_diameter.data,
...
*/
void TwoStepABDMobilityFieldGPU::integrateStepTwo(unsigned int timestep)
    {
    // there is no step 2
    }

void export_TwoStepABDMobilityFieldGPU(py::module& m)
    {
    py::class_<TwoStepABDMobilityFieldGPU, std::shared_ptr<TwoStepABDMobilityFieldGPU> >(m, "TwoStepABDMobilityFieldGPU", py::base<TwoStepABDMobilityField>())
