...
#include "TwoStepABDMobilityFieldGPU.cuh"
...
/*! \file TwoStepABDMobilityFieldGPU.cu
    \brief Defines GPU kernel code for Brownian integration with a mobility field on the GPU. Used by TwoStepABDMobilityFieldGPU.
*/
...
/*! \param d_pos array of particle positions and types
    ...
    \param alpha Decay length
    \param v0 Propulsion speed 
    \param offset Offset of this GPU into group indices
    \param d_td_field If set true, the translational mobility will depend on the distance from the wall
    \param d_rd_field If set true, the rotational mobility will depend on the distance from the wall
...
*/
extern "C" __global__
void gpu_active_brownian_mobilityfield_step_one_kernel(Scalar4 *d_pos,
                                  Scalar4 *d_vel,
                                  int3 *d_image,
                                  const BoxDim box,
    ...
    if (local_idx < nwork)
	    ...
	    RandomGenerator rng(RNGIdentifier::TwoStepABDMobilityField, seed, ptag, timestep);
		...
		else
		{
            ...
            Scalar h = fast::sqrt( (rcon*rcon) - (r+0.5)*(r+0.5) ); // particle diameter is taken to be unity
		    ...
		    if (d_td_field)
			{
                ...
                dh_dy = - ry*(r+0.5) / (h*r);
                mu = mu0*tanh(h/alpha);
                dmu_dh = mu0 * ( 1 - tanh(h/alpha)*tanh(h/alpha) )/alpha;
			}
            ...
            Scalar randy = stdnormal(rng);
		   	Scalar coeff_rand = fast::sqrt(Scalar(2.0)*T*mu/deltaT);
		    if (d_noiseless_t)
			{
			...
			Scalar vxl = v0*fast::cos(theta) + mu*net_force.x + T*dmu_dh*dh_dx;
			Scalar vyl = v0*fast::sin(theta) + mu*net_force.y + T*dmu_dh*dh_dy;
			
		    // update position
			postype.x += (vxl + coeff_rand*randx)*deltaT;
			postype.y += (vyl + coeff_rand*randy)*deltaT;
        ...
        if (aniso)
            ...
            if (gamma_r.x > 0 || gamma_r.y > 0 || gamma_r.z > 0)
				...
				else
                {
                    ...
                    Scalar mu_r = mu_r0; 
                    Scalar sigma_r = fast::sqrt(Scalar(2.0)*T/(deltaT*mu_r));
                    ...
                    if (d_rd_field)
                    {
                        ...
                        Scalar r = fast::sqrt(r2);
                        Scalar h = fast::sqrt( (rcon*rcon) - (r+0.5)*(r+0.5) ); // particle diameter is taken to be unity
                        mu_r = mu_r0*tanh(h/alpha);
                        sigma_r = fast::sqrt(Scalar(2.0)*T/(deltaT*mu_r));
                    }
				    ...
				    Scalar ort_x = ori_x + (t.z*ori_y*mu_r)*deltaT;
				    Scalar ort_y = ori_y - (t.z*ori_x*mu_r)*deltaT;
				    ...
				    Scalar ang_final = atan2(ort_y,ort_x) + (bf_torque*mu_r)*deltaT;
...
/*! \param d_pos array of particle positions and types
    ...
    \param d_inertia Device array of moment of inertial of each particle
    \param d_angmom Device array of transformed angular momentum quaternion of each particle (see online documentation)
    \param langevin_args Collected arguments for gpu_brownian_mobilityfield_step_one_kernel()
    \param aniso If set true, the system would go through rigid body updates for its orientation
    \param deltaT Amount of real time to step forward in one time step
    \param D Dimensionality of the system
    \param d_noiseless_t If set true, there will be no translational noise (random force)
    ...
    This is just a driver for gpu_brownian_mobilityfield_step_one_kernel(), see it for details.
*/
cudaError_t gpu_active_brownian_mobilityfield_step_one(Scalar4 *d_pos,
    ...
    for (int idev = gpu_partition.getNumActiveGPUs() - 1; idev >= 0; --idev)
        ...
        // run the kernel
        gpu_active_brownian_mobilityfield_step_one_kernel<<< grid, threads, (unsigned int)(sizeof(Scalar)*langevin_args.n_types + sizeof(Scalar3)*langevin_args.n_types)>>>
                                    (d_pos,
                                     d_vel,
                                     d_image,
                                     box,
